...
public class PixmapHelper implements Disposable {
	...
	public void eraseRectangle(float x, float y, float radius) {
		eraseRectangle(x, y, radius, TransparentColor);
	}

	public void eraseRectangle(float x, float y, float radius, Color color) {
		if (lastModification == modifications.length)
			return;
		...
		pixmap.fillRectangle(newX, newY, newRadius, newRadius);
		Pixmap.setBlending(blending);
	 ...
	 */
	public void reload() {
		texture.load(new PixmapTextureData(pixmap, pixmap.getFormat(), false,
				false));
	}

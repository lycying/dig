...
public class RedBallSaveTheWorld extends Applet implements Runnable {
	...
	public void run() {
		...
		// Walls
		BufferedImage wallImage = new BufferedImage(TILE_SIZE, TILE_SIZE,
				BufferedImage.TYPE_INT_ARGB);
		Graphics wallGraphics = wallImage.getGraphics();
		for (x = 0; x < TILE_SIZE; x++) {
			for (y = 0; y < TILE_SIZE; y++) {
				int randomNumber = random.nextInt(100) + 80;
				wallGraphics.setColor(new Color(randomNumber, randomNumber,
						randomNumber));
		...
		// Ladders
		BufferedImage ladderImage = new BufferedImage(TILE_SIZE, TILE_SIZE,
				BufferedImage.TYPE_INT_ARGB);
		...
		// Loading level-box
		BufferedImage loadBoxImage = new BufferedImage(200, 40,
				BufferedImage.TYPE_INT_ARGB);
		Graphics loadBoxGfx = loadBoxImage.getGraphics();
		for (i = 0; i < 40; i++) {
			loadBoxGfx.setColor(new Color(255 - (i * 3), 255 - (i * 3),
					255 - (i * 3)));
		...
		while (BUTTON_FIRE_PRESSED == NO) {
			...
			g.drawString("Jens Stääf 2012", 293, 50);
		...
		while (true) {
			...
			if (levelCompleted == YES) {
				...
				if (levelIndex < 10)
					currentLevel = levels.substring(((levelIndex - 1) * 330),
							((levelIndex - 1) * 330) + 330);
				else
					currentLevel = levels.substring(((levelIndex - 10) * 330),
							((levelIndex - 10) * 330) + 330);
				...
				bgImage2 = new BufferedImage(WINDOW_WIDTH, WINDOW_HEIGHT,
						BufferedImage.TYPE_INT_ARGB);
				Graphics backgroundGfx = bgImage2.getGraphics();
				for (y = 0; y < WINDOW_HEIGHT; y++) {
					backgroundGfx.setColor(new Color(255 - (y / x),
							255 - (y / i), 255 - (y / a)));
					backgroundGfx.drawLine(0, y, WINDOW_WIDTH, y);
				}

				// Render map
				mapImage2 = new BufferedImage(WINDOW_WIDTH, WINDOW_HEIGHT,
						BufferedImage.TYPE_INT_ARGB);
				...
				for (y = 0; y < MAP_HEIGHT; y++) {
					for (x = 0; x < MAP_WIDTH; x++) {
						if (currentLevel.charAt((y * MAP_WIDTH) + x) == TILE_WALL) {
							gfx.drawImage(wallImage, x * TILE_SIZE, y
									* TILE_SIZE, null);

							if (y != 0
									&& currentLevel
											.charAt(((y - 1) * MAP_WIDTH) + x) != TILE_WALL)
								gfx.drawLine(x * TILE_SIZE, y * TILE_SIZE, x
										* TILE_SIZE + TILE_SIZE, y * TILE_SIZE); // Upper
							// line
							if (y != MAP_HEIGHT - 1
									&& currentLevel
											.charAt(((y + 1) * MAP_WIDTH) + x) != TILE_WALL)
								gfx.drawLine(x * TILE_SIZE, y * TILE_SIZE
										+ TILE_SIZE, x * TILE_SIZE + TILE_SIZE,
										y * TILE_SIZE + TILE_SIZE); // Lower
							// line
							if (x != 0
									&& currentLevel.charAt((y * MAP_WIDTH)
											+ (x - 1)) == TILE_SPACE)
								gfx.drawLine(x * TILE_SIZE, y * TILE_SIZE, x
										* TILE_SIZE, y * TILE_SIZE + TILE_SIZE); // Left
							// line
							if (x != MAP_WIDTH - 1
									&& currentLevel.charAt((y * MAP_WIDTH)
											+ (x + 1)) == TILE_SPACE)
								gfx.drawLine(x * TILE_SIZE + TILE_SIZE, y
										* TILE_SIZE, x * TILE_SIZE + TILE_SIZE,
										y * TILE_SIZE + TILE_SIZE); // Right
							// line
						} else if (currentLevel.charAt((y * MAP_WIDTH) + x) == TILE_LADDER) {
							gfx.drawImage(ladderImage, x * TILE_SIZE, y
									* TILE_SIZE, null);
						}
					}
				}
			...
			do {
				...
				// Input
				if (levelLoaded == YES) {
					if (PLAYER_MOVING_LEFT == YES && player_climbing == NO
							&& player_falling == NO) {
						if (currentLevel.charAt(((player_y / 32) * MAP_WIDTH)
								+ (player_x / 32)) == TILE_SPACE
								|| currentLevel
										.charAt(((player_y / 32) * MAP_WIDTH)
												+ (player_x / 32)) == TILE_LADDER)
							player_x -= PLAYER_SPEED;
						player_direction = DIRECTION_LEFT;
					} else if (PLAYER_MOVING_RIGHT == 1
							&& player_climbing == NO && player_falling == NO) {
						if (currentLevel.charAt(((player_y / 32) * MAP_WIDTH)
								+ (player_x / 32 + 1)) == TILE_SPACE
								|| currentLevel
										.charAt(((player_y / 32) * MAP_WIDTH)
												+ (player_x / 32 + 1)) == TILE_LADDER)
							player_x += PLAYER_SPEED;
						player_direction = DIRECTION_RIGHT;
					}
					if (PLAYER_MOVING_UP == YES) {
						if ((currentLevel.charAt(((player_y / 32) * MAP_WIDTH)
								+ (player_x / 32)) == TILE_LADDER || currentLevel
								.charAt(((player_y / 32) * MAP_WIDTH)
										+ (player_x / 32 + 1)) == TILE_LADDER)
								|| (player_climbing == YES && (currentLevel
										.charAt(((player_y / 32 + 1) * MAP_WIDTH)
												+ (player_x / 32)) == TILE_LADDER || currentLevel
										.charAt(((player_y / 32 + 1) * MAP_WIDTH)
												+ (player_x / 32 + 1)) == TILE_LADDER))) {
					...
					if (BUTTON_FIRE_PRESSED == YES && player_climbing == NO
							&& player_bulletCounter >= TIME_BETWEEN_BULLETS) {
				...
				for (i = 0; i < numberOfActiveEnemies; i++) {
					...
					if (enemy_health[i] > 0) {
						if (currentLevel
								.charAt(((enemy_y[i] / 32 + 1) * MAP_WIDTH)
										+ (enemy_x[i] / 32 + 1)) == TILE_SPACE
								&& currentLevel
										.charAt(((enemy_y[i] / 32 + 1) * MAP_WIDTH)
												+ (enemy_x[i] / 32)) == TILE_SPACE) {
							enemy_y[i] += ENEMY_SPEED;
							enemy_climbing[i] = NO;
						} else if (player_y != enemy_y[i]
								&& currentLevel
										.charAt(((enemy_y[i] / 32) * MAP_WIDTH)
												+ (enemy_x[i] / 32)) == TILE_LADDER
								&& random.nextInt(17) == 1) {
						...
						if (((enemy_x[i] >= player_x && enemy_x[i] <= player_x
								+ TILE_SIZE) || (enemy_x[i] <= player_x && enemy_x[i] >= player_x
								- TILE_SIZE))
								&& ((enemy_y[i] >= player_y && enemy_y[i] <= player_y
										+ TILE_SIZE) || (enemy_y[i] <= player_y && enemy_y[i] >= player_y
										- TILE_SIZE))) {
							levelIndex--;
							levelCompleted = YES;
							...
							for (y = 0; y < 20; y++) {
								if (activeParticles < MAX_NUMBER_OF_PARTICLES) {
									...
									particle_xVelocity[activeParticles] = random
											.nextInt(6) - 3;
									particle_yVelocity[activeParticles] = random
											.nextInt(6) - 3;
						...
						for (x = 0; x < MAX_NUMBER_OF_ACTIVE_BULLETS; x++) {
							if (bullet_direction[x] != DIRECTION_NONE
									&& ((enemy_x[i] >= bullet_x[x] && enemy_x[i] <= bullet_x[x]
											+ TILE_SIZE) || (enemy_x[i] <= bullet_x[x] && enemy_x[i] >= bullet_x[x]
											- TILE_SIZE))
									&& ((enemy_y[i] >= bullet_y[x] && enemy_y[i] <= bullet_y[x]
											+ TILE_SIZE) || (enemy_y[i] <= bullet_y[x] && enemy_y[i] >= bullet_y[x]
											- TILE_SIZE))) {
								enemy_health[i]--;
				...
				if (player_falling == YES)
					player_y += 2;
				if (currentLevel.charAt(((player_y / 32 + 1) * MAP_WIDTH)
						+ (player_x / 32 + 1)) == TILE_SPACE
						&& currentLevel
								.charAt(((player_y / 32 + 1) * MAP_WIDTH)
										+ (player_x / 32)) == TILE_SPACE)
			...
			g.fillOval(player_x + (balls_height / 2), player_y - balls_height,
					32 - balls_height, 32 + balls_height);

			g.setColor(new Color(225, 0, 225));
			g.fillOval(player_x + (balls_height / 2), player_y - balls_height,
					28 - balls_height, 28 + balls_height);

			g.setColor(Color.BLACK);
			g.drawOval(player_x + (balls_height / 2), player_y - balls_height,
					32 - balls_height, 32 + balls_height);

			g.setColor(new Color(255, 0, 255));
			g.fillOval(player_x + (balls_height / 2) + 3, player_y
					- balls_height + 3, 16 - balls_height, 16 + balls_height);

			// Draw enemies
			for (i = 0; i < numberOfActiveEnemies; i++) {
				if (enemy_health[i] > 0) {
					g.setColor(new Color(0, 10 - (enemy_health[i] * -35), 0));
					g.fillOval(enemy_x[i] + (balls_height / 2), enemy_y[i]
							- balls_height, 32 - balls_height,
							32 + balls_height);

					g.setColor(new Color(0, 50 - (enemy_health[i] * -35), 0));
					g.fillOval(enemy_x[i] + (balls_height / 2), enemy_y[i]
							- balls_height, 28 - balls_height,
							28 + balls_height);

					g.setColor(Color.BLACK);
					g.drawOval(enemy_x[i] + (balls_height / 2), enemy_y[i]
							- balls_height, 32 - balls_height,
							32 + balls_height);

					g.setColor(new Color(0, 80 - (enemy_health[i] * -35), 0));
					g.fillOval(enemy_x[i] + (balls_height / 2) + 3, enemy_y[i]
							- balls_height + 3, 16 - balls_height,
							16 + balls_height);
				}
			}
			...
			for (i = 0; i < MAX_NUMBER_OF_ACTIVE_BULLETS; i++) {
				if (bullet_direction[i] != DIRECTION_NONE) {
					for (x = 0; x < 16; x++) {
						g.setColor(new Color(255 - random.nextInt(255),
								255 - random.nextInt(255), 255 - random
										.nextInt(255)));
						g.fillRect(bullet_x[i] + x * 2, bullet_y[i] + 9
								+ random.nextInt(6) - 6, 4, 2);
					}
				} else {
					g.setColor(Color.WHITE);

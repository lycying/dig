...
public class Pitfall4K extends Applet implements Runnable {
  ...
  public void run() {
    ...
    for(i = 0; i < 50; i++) {
      ...
      for(y = 0; y < j; y++) {
        for(x = 0; x < 8; x++, k++) {
          z = (S.charAt(k) == ' ') ? 0
              : (0xFF000000 | COLORS[S.charAt(k) - 'a']);
    ...
    while(true) {
      ...
      do {
        ...
        if (attractMode) {
          ...
          if (++crocodileTimer == 210) {
            crocodileTimer = 0;
            crocodileSprite = crocodileSprite != SPRITE_CROCODILE_0
                ? SPRITE_CROCODILE_0 : SPRITE_CROCODILE_1;
          }
          ...
          if (harrySwinging) {
            ...
            if (((jumpReleased && a[VK_JUMP])
                || (harryClimbedDown && (a[VK_LEFT] || a[VK_RIGHT])))
                    && harryY == LADDER_TOP_Y) {
              jumpReleased = false;
              harryJumping = true;
              harryVy = JUMP_SPEED;
            ...
            if (harryCrushed == 0) {
              if (a[VK_LEFT]) {
                harryDirection = LEFT;
                if ((timer & 1) == 0) {
                  if ((harryX != 37 && harryX != 89)
                      || harryY <= UPPER_FLOOR_Y
                      || harryY >= UPPER_FLOOR_LOWER_Y) {
                    harryX--;
                  }
                }
                if (++harryRunTimer == 4) {
            ...
            if (!harryJumping && scene <= SCENE_LADDER_AND_HOLES) {
              if (harryX >= 65 && harryX <= 71
                  && (harryY == UPPER_FLOOR_Y || a[VK_UP])) {
              ...
              } else if (harryY == UPPER_FLOOR_Y
                  && scene == SCENE_LADDER_AND_HOLES) {
                if ((harryX >= 37 && harryX <= 47)
                    || (harryX >= 89 && harryX <= 99)) {
                  harryJumping = true;
                  harryY++;
          ...
          // Harry sinking test
          harrySinking = harryY == UPPER_FLOOR_Y
              && pit && pitOffset < 5
              && harryX >= 36 && harryX <= 100              
              && (image.getRGB(harryX, 119) & 0xFFFFFF) != DARK_YELLOW
              && (image.getRGB(harryX + 7, 119) & 0xFFFFFF) != DARK_YELLOW;
          if (crocodiles) {
            for(i = 0; i < 3; i++) {
              if (harryX <= 56 + (i << 4) && harryX >= 48
                  + 6 * (crocodileSprite - SPRITE_CROCODILE_0) + (i << 4)) {
                harrySinking = false;
              }
            }
          }
          ...
          for(i = queue.size() - 1; i >= 0; i--) {
            ...
            if (j == OBJECT_TYPE_LOG) {
              ...
              if ((timer & 7) == 0) {
                ...
                object[OBJECT_SPRITE_INDEX] = object[OBJECT_SPRITE_INDEX]
                    != SPRITE_SCORPION_0 ? SPRITE_SCORPION_0
                        : SPRITE_SCORPION_1;
              }
            }
            ...
            if ((timer & 1) == 0 && Math.random() < 0.5f) {
              ...
              if (j == OBJECT_TYPE_FIRE) {
                object[OBJECT_SPRITE_DIRECTION] ^= 1;
              } else if (j == OBJECT_TYPE_RATTLESNAKE
                  || j == OBJECT_TYPE_SILVER_BRICK
                  || j == OBJECT_TYPE_GOLD_BRICK) {
                object[OBJECT_SPRITE_INDEX] =
                    object[OBJECT_SPRITE_INDEX] != object[OBJECT_SPRITE_INDEX_2]
                        ? object[OBJECT_SPRITE_INDEX_2]
                        : object[OBJECT_SPRITE_INDEX_2] + 1;
              }
            }

            if (!harrySwinging
                && harryX >= x - 4 && harryX <= x + 4
                && harryY >= y - 16 + (j == OBJECT_TYPE_SCORPION ? 6 : 0)
                && harryY <= y + 1) {
              // Harry collides with object

              if (j == OBJECT_TYPE_FIRE || j == OBJECT_TYPE_RATTLESNAKE
                  || j == OBJECT_TYPE_SCORPION) {
                // Harry collides with deadly object
                restartDelay = 60;
              } else if (j >= OBJECT_TYPE_MONEY_BAG
                  && j <= OBJECT_TYPE_DIAMOND_RING) {
          ...
          if (harryX > 150) {
            ...
            for(i = harryY > UPPER_FLOOR_Y ? 3 : 1; i > 0; i--) {              
              screen = 0xFF & ((screen << 1) | ((1 & (screen >> 3))
                  ^ (1 & (screen >> 4)) ^ (1 & (screen >> 5))
                  ^ (1 & (screen >> 7))));
              if (++screenIndex > 254) {
                screenIndex = 0;
              }
            ...
            for(i = harryY > UPPER_FLOOR_Y ? 3 : 1; i > 0; i--) {              
              screen = 0xFF & ((screen >> 1) | ((1 & (screen >> 4))
                  ^ (1 & (screen >> 5))^ (1 & (screen >> 6))
                  ^ (1 & screen)) << 7);
              if (--screenIndex < 0) {
                screenIndex = 254;
              }
          ...
          if (i != screen || resetScreen == true) {
            ...
            // create vine
            vine = scene == SCENE_TAR_WITH_VINE
                || scene == SCENE_QUICKSAND_WITH_VINE
                || scene == SCENE_SHIFTING_TAR_WITH_VINE
                || (scene == SCENE_CROCODILES && (screen & 2) == 2);

            // create pit
            pitSprite = scene == SCENE_TAR_WITH_VINE
                || scene == SCENE_SHIFTING_TAR_WITH_TREASURE
                || scene == SCENE_SHIFTING_TAR_WITH_VINE
                    ? SPRITE_TAR : SPRITE_WATER;
            ...
            if (scene == SCENE_SHIFTING_TAR_WITH_TREASURE) {
              ...
              if (!collectedTreasures[screenIndex]) {
                ...
                object[OBJECT_Y] = 111;
                object[OBJECT_SPRITE_INDEX_2]
                    = object[OBJECT_SPRITE_INDEX] = j == 0 ? SPRITE_MONEY_BAG
                        : j == 1 ? SPRITE_SILVER_BAR_0 : j == 2
                            ? SPRITE_GOLD_BAR_0 : SPRITE_DIAMOND_RING;
              ...
              for(i = 0; i < ((j == ITEM_2_CLOSE_ROLLING_LOGS
                  || j == ITEM_2_FAR_ROLLING_LOGS) ? 2
                      : ((j == ITEM_3_ROLLING_LOGS
                          || j == ITEM_3_STATIONARY_LOGS) ? 3 : 1)); i++) {
                object = new int[OBJECT_ARRAY_SIZE];
                queue.add(object);
                object[OBJECT_X] = ((i << (j == ITEM_2_CLOSE_ROLLING_LOGS
                    ? 4 : 5)) + 116) % 160;
                object[OBJECT_Y] = 111;
                object[OBJECT_SPRITE_INDEX] = object[OBJECT_SPRITE_INDEX_2]
                    = j == 6 ? SPRITE_FIRE : j == 7
                        ? SPRITE_RATTLESNAKE_0 : SPRITE_LOG;
                object[OBJECT_TYPE] = j == 6 ? OBJECT_TYPE_FIRE : j == 7
                    ? OBJECT_TYPE_RATTLESNAKE : OBJECT_TYPE_LOG;
      ...
      if (scene <= SCENE_LADDER_AND_HOLES) {
        ...
        // draw brick wall
        g.drawImage(sprites[SPRITE_BRICK_WALL][0],
            wallOnLeft ? 10 : 128, 142, null);
        g.drawImage(sprites[SPRITE_BRICK_WALL][0],
            wallOnLeft ? 10 : 128, 158, null);
      }
      ...
      // draw Harry
      g.drawImage(sprites[harrySprite][harryDirection],
          harryX, harryOffsetY + (int)harryY, null);
      ...
      // draw tree leaves
      for(i = 0; i < 6; i++) {
        g.drawImage(sprites[SPRITE_LEAVES_3 - background][1 - (i & 1)],
            (i << 5) - 24, 51, 32, 8, null);
      }
      ...
      for(i = queue.size() - 1; i >= 0; i--) {
        object = queue.get(i);
        g.drawImage(sprites[object[OBJECT_SPRITE_INDEX]]
            [object[OBJECT_SPRITE_DIRECTION]],
                object[OBJECT_X], object[OBJECT_Y], null);
      }
      ...
      for(i = 0; i < 6; i++) {
        g.drawImage(sprites[SPRITE_COPYRIGHT_0 + i][0], 
            13 + (i << 3), 183 - copyrightOffset, null);
      }
      g.setClip(null);
      ...
      if (hintsEnabled) {
        ...
        i = (harryY < 124 && (screenIndex == 11
            ...
            || screenIndex == 235))
            ? SPRITE_ARROW_1
                : (harryY >= 124 && (screenIndex == 25
                || screenIndex == 92
                || screenIndex == 179
                || screenIndex == 209
                || screenIndex == 226
                || screenIndex == 245)) ? SPRITE_ARROW_2 : SPRITE_ARROW_0;
        j = harryY < 124
            && ((screenIndex >= 0 && screenIndex < 11)
                || (treasures == 30 && screenIndex >= 226 && screenIndex <= 228)
                || (treasures == 13 && screenIndex >= 92 && screenIndex <= 95)
                || (treasures == 1 && screenIndex >= 25 && screenIndex <= 28))
                    ? RIGHT : LEFT;
